...
// const passwordHash =
//   '$2b$10$0Jvk7/fBmMI/mISeI1p2zus/UkRG0dWrlTWvyPl6h1P7o3krvjZae';
const passwordHash = '$2b$10$uJfXiQbtNgBJetgzwjG9/ON/cOuoIsOEmps7qB7lakRa5VhO0moC.';
...
export const checkLoginInfo = (user, pass) =>
  user === email && bcrypt.compareSync(pass, passwordHash);
  ...
  if (!verifyToken(req.headers.authorization)) {
    return next(
      new UnauthorizedError('credentials_bad_format', {
        message: "Couldn't verify the authorization token",
      }),
    );
...
export const login = (req, res, next) => {
  ...
  if (!checkLoginInfo(jsonData.email, jsonData.password)) {
    ...
    return next(
      new UnauthorizedError('credentials_invalid', {
        message: "User name and password don't match our records",
      }),
    );
